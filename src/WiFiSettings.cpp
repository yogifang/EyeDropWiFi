...
struct WiFiSettingsParameter {
  ...
  bool store() {
    return (name && name.length()) ? spurt(filename(), value) : true;
  }
  void fill() {
    if (name && name.length())
...
struct WiFiSettingsInt : WiFiSettingsParameter {
  ...
  String html() {
    String h =
        F("<p><label>{label}:<br><input type=number step=1 min={min} max={max} "
          "name='{name}' value='{value}' placeholder='{init}'></label>");
    h.replace("{name}", html_entities(name));
    h.replace("{value}", html_entities(value));
    h.replace("{init}", html_entities(init));
    h.replace("{label}", html_entities(label));
...
struct WiFiSettingsBool : WiFiSettingsParameter {
  ...
  String html() {
    String h = F("<p><label class=c><input type=checkbox name='{name}' "
                 "value=1{checked}> {label} ({default}: {init})</label>");
    h.replace("{name}", html_entities(name));
...
String WiFiSettingsClass::string(const String &name, const String &init,
                                 const String &label) {
  begin();
...
String WiFiSettingsClass::string(const String &name, unsigned int max_length,
                                 const String &init, const String &label) {
  String rv = string(name, init, label);
  params.back()->max = max_length;
  return rv;
}

String WiFiSettingsClass::string(const String &name, unsigned int min_length,
                                 unsigned int max_length, const String &init,
                                 const String &label) {
  String rv = string(name, init, label);
...
long WiFiSettingsClass::integer(const String &name, long init,
                                const String &label) {
  begin();
...
long WiFiSettingsClass::integer(const String &name, long min, long max,
                                long init, const String &label) {
...
bool WiFiSettingsClass::checkbox(const String &name, bool init,
                                 const String &label) {
  begin();
...
void WiFiSettingsClass::html(const String &tag, const String &contents,
                             bool escape) {
  begin();
...
void WiFiSettingsClass::portal() {
  ...
  http.on("/", HTTP_GET, [this, &http, &num_networks, &redirect]() {
    ...
    http.sendContent(
        F("</title>"
          ...
          "a:link{color:#000} "
          "label{clear:both}"
          "select,input:not([type^=c]){display:block;width:100%;border:1px "
          "solid #444;padding:.3ex}"
          "input[type^=s]{display:inline;width:auto;background:#de1;padding:"
          "1ex;border:1px solid #000;border-radius:1ex}"
    ...
    for (int i = 0; i < num_networks; i++) {
      ...
      opt.replace("{1x}",
                  mode == WIFI_AUTH_WPA2_ENTERPRISE ? _WSL_T.dot1x : F(""));
    ...
    if (!found && current.length()) {
      String opt = F("<option value='{ssid}' selected>{ssid} (&#x26a0; not in "
                     "range)</option>");
    ...
    http.sendContent(
        F("</select></label> <a href=/rescan onclick=\"this.innerHTML='"));
 ...
 http.sendContent(_WSL_T.url_restapi);
  ...
  http.on("/", HTTP_POST, [this, &http]() {
    bool ok = true;
    if (!spurt("/wifi-ssid", http.arg("ssid")))
      ok = false;

    if (WiFiSettingsLanguage::multiple()) {
      if (!spurt("/WiFiSettings-language", http.arg("language")))
        ok = false;
    ...
    Serial.println(pw);
    if (pw != "##**##**##**") {
      if (!spurt("/wifi-password", pw))
        ok = false;
    }


    String restapi = http.arg("url_restapi");
    Serial.println(restapi);
    if(!spurt("/rest-api", restapi))
      ok = false;
...
bool WiFiSettingsClass::connect(bool portal, int wait_seconds) {
  ...
  while (WiFi.status() != WL_CONNECTED &&
         (wait_seconds < 0 ||
          (millis() - starttime) < (unsigned)wait_seconds * 1000)) {
    Serial.print(".");
...
void WiFiSettingsClass::begin() {
  ...
  if (user_language.length() &&
      WiFiSettingsLanguage::available(user_language)) {
